...
Created with **MindCraft** + **Gemini AI**
...
---
*Generated by MindCraft with Gemini AI*
"""
...
def create_and_push_repo(token):
    ...
    # Create repository
    import time
    repo_name = f"mindcraft-{int(time.time())}"
    ...
    create_response = requests.post(
        ...
        json={
            'name': repo_name,
            'auto_init': True,
            'description': 'Created with MindCraft'
        }
    )
...
"""
    ...
    update_data = {
        'message': 'Initial commit from MindCraft',
        'content': base64.b64encode(readme_content.encode()).decode()
    }
    if sha:
        update_data['sha'] = sha
